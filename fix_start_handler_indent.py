...
print("🔧 FIX ERRORE INDENTAZIONE start_handler.py")
print("=" * 50)

# 1. Leggi il file
print("\n1️⃣ Lettura start_handler.py...")
with open('handlers/start_handler.py', 'r') as f:
    lines = f.readlines()

# 2. Mostra le righe intorno alla 108
print("\n2️⃣ Analisi righe 105-115...")
for i in range(max(0, 105), min(len(lines), 115)):
    # Mostra spazi
    line_display = lines[i].replace(' ', '·').replace('\t', '→')
    print(f"{i+1}: {line_display}", end='')

# 3. Correggi l'indentazione
print("\n\n3️⃣ Correzione automatica...")

# Trova la funzione send_dashboard e sistema l'indentazione
fixed_lines = []
in_send_dashboard = False
base_indent = 0

for i, line in enumerate(lines):
    if 'async def send_dashboard' in line:
        in_send_dashboard = True
        base_indent = len(line) - len(line.lstrip())
        fixed_lines.append(line)
    elif in_send_dashboard and line.strip() and not line.startswith(' '):
        # Fine della funzione
        in_send_dashboard = False
        fixed_lines.append(line)
    elif in_send_dashboard:
        # Dentro la funzione
        if line.strip():
            # Linea con codice
            stripped = line.strip()
            if i == 107:  # La riga problematica (108 in editor)
                # Deve avere indentazione base + 4
                fixed_line = ' ' * (base_indent + 4) + stripped + '\n'
                fixed_lines.append(fixed_line)
                print(f"✅ Corretta riga {i+1}: {stripped[:50]}...")
            else:
                # Mantieni indentazione relativa
                current_indent = len(line) - len(line.lstrip())
                if current_indent < base_indent + 4:
                    # Troppo poco indentata
                    fixed_line = ' ' * (base_indent + 4) + stripped + '\n'
                    fixed_lines.append(fixed_line)
                else:
                    fixed_lines.append(line)
...
result = subprocess.run(['python3', '-m', 'py_compile', 'handlers/start_handler.py'], capture_output=True)
if result.returncode == 0:
    ...
    # Se ancora errori, ricrea il file da zero
    print("\n6️⃣ Ricreo start_handler.py da zero...")
...
'''
    ...
    with open('handlers/start_handler.py', 'w') as f:
        f.write(start_handler_clean)
    
    print("✅ start_handler.py ricreato da zero")

# 6. Commit e push
print("\n7️⃣ Commit e push...")
subprocess.run("git add handlers/start_handler.py", shell=True)
subprocess.run('git commit -m "fix: corretto errore indentazione in start_handler.py"', shell=True)
